...
```{r, emp-jaccard-dotplot-w-annotations}
jaccard_merged_df %>%
  ggplot(.) +
  aes(x = z_emp, fill = group) +
  #geom_histogram(bins = 20) +
  geom_dotplot(dotsize = .33, binwidth = .25) +
  geom_vline(xintercept = qnorm(1-.5*(.0001)), color = "black") +
  #geom_vline(xintercept = qnorm(1-.5*(.001)), color = "black", alpha = 0.65) +
  geom_vline(xintercept = qnorm(1-.5*(.01)), color = "black", alpha = 0.45) +
  ylab(element_blank()) +
  theme(axis.text.y = element_blank()) +
  theme(axis.ticks.y = element_blank()) +
  annotate("text", x = 4.6, y = 50, label = "p=.0001") +
  annotate("text", x = 3.1, y = 50, label = "p=.01") +
  facet_grid(group ~ .)
```
...
jaccard_merged_df %>%
  ...
  theme(axis.text.y = element_blank()) +
  theme(axis.ticks.y = element_blank()) +
  annotate("text", x = 4.6, y = 50, label = "p=.0001", size = 3) +
  annotate("text", x = 3.1, y = 50, label = "p=.01", size = 3) +
  facet_grid(group ~ .)
```
