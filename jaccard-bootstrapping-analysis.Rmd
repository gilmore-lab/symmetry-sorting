...
make_aggregate_perm_jaccard_df <- function(wp_group = "P1",
  ...
  these_csvs <- list.files(input_dir, paste0("^", wp_group), full.names = TRUE)
...
P1_perm_df %>%
  ...
  aes(x = Jaccard) +
  geom_histogram() +
  facet_wrap(~ exemplar_pair, ncol = 10)
```

I'll need to reformat the exemplar pair identifiers for this plot to work.