...
- The function `analysis/load.wp()` loads from Databrary specific exemplars from Databrary and saves a local copy of the image file in `img/`.
  - ROG ran this recently to create local copies of the images.
  - The `analysis/compute.image.metric()` function computes an image metric given a particular function specified as the `FUN` parameter in the function call. The default is `ssim()`.
- It looks like a useful step would be to combine the behavioral data with the image similarity data based on different metrics.
- ROG created `data-surgery.Rmd` to create new CSV data files with the image statistics. The `analysis/data/wallpapers-sorting-image.csv` file contains Jaccard indices from the sorting task and two 'image-level' metrics. The `analysis/data/nsets.csv` file contains data about the size and number of categories by individual participant.
...
- 2d MDS on the full-scale wallpapers using the image-wise dot product is in [HTML]https://rawgit.com/gilmore-lab/symmetry-sorting/master/dotprod-mds-analysis.html) and [Rmd](../dotprod-mds-analysis.Rmd).
