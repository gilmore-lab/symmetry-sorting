...
3. Are there wallpaper groups with exemplar pairs that have especially high Jaccard indices?
...
Let's view the number of exemplars per set per participant. That is, each dot in the plot below is one set contributed by one participant.
...
It may be useful to examine those exemplar pairs that have especially large Jaccard indices.

As a starting point for exploration, we examine those exemplars with Jaccard indices > some threshold. Here, we choose `r (j_thresh=.35)`.

```{r}
jaccard_plot_4 <- jaccard %>%
  filter(., Jaccard > j_thresh) %>%
  ggplot(.) +
  aes(x = Jaccard, fill = Group) +
  geom_boxplot()
jaccard_plot_4
